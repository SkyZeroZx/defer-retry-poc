...
function createRootTView(rootSelectorOrNode, componentDef, componentBindings, directives) {
  const tAttributes = rootSelectorOrNode ? ['ng-version', '22.1.0-next.0+sha-ca11988-with-local-changes'] : extractAttrsAndClassesFromSelector(componentDef.selectors[0]);
...
function setClassMetadataAsync(type, dependencyLoaderFn, metadataSetterFn) {
  const componentClass = type;
  componentClass[ASYNC_COMPONENT_METADATA_FN] = () => Promise.all(dependencyLoaderFn().map(dep => dep())).then(dependencies => {
...
const RETRY_ATTEMPTS_REMAINING = 10;
...
function getBaseDocUrl() {
  ...
  const isPreRelease = full.includes('-next') || full.includes('-rc') || full === '22.1.0-next.0+sha-ca11988-with-local-changes';
...
function ɵɵdeferEnableRetry(tDetails, retryCount) {
...
const defaultDeferBlockRetryHandler = (load, ctx) => ctx.attempt === 0 ? load() : ctx.retry();
function reloadDeferDependencyWithCacheBust(load, attempt) {
  let source;
  try {
...
function appendRetryQueryParam(url, attempt) {
  const fragmentIdx = url.indexOf('#');
  const fragment = fragmentIdx >= 0 ? url.slice(fragmentIdx) : '';
  const base = fragmentIdx >= 0 ? url.slice(0, fragmentIdx) : url;
  const sep = base.includes('?') ? '&' : '?';
  return `${base}${sep}ngRetry=${attempt}${fragment}`;
}
const DEFER_BLOCK_RETRY_HANDLER = new InjectionToken(typeof ngDevMode !== 'undefined' && ngDevMode ? 'DEFER_BLOCK_RETRY_HANDLER' : '', {
  factory: () => defaultDeferBlockRetryHandler
});
function provideDeferBlockRetryHandler(handler) {
  return makeEnvironmentProviders([{
...
function triggerResourceLoading(tDetails, lView, tNode) {
  ...
  tDetails.loadingPromise = Promise.allSettled(dependencies).then(results => {
    let failed = false;
...
function isDeferDependencyLoader(dep) {
  return typeof dep === 'function' && getComponentDef(dep) === null && getDirectiveDef(dep) === null && getPipeDef$1(dep) === null;
}
...
function ɵɵdefer(index, primaryTmplIndex, dependencyResolverFn, loadingTmplIndex, placeholderTmplIndex, errorTmplIndex, loadingConfigIndex, placeholderConfigIndex, enableTimerScheduling, flags, enableRetry, retryCount) {
  const lView = getLView();
  const tView = getTView();
  const adjustedIndex = index + HEADER_OFFSET;
  ...
  const lDetails = [null, DeferBlockInternalState.Initial, null, null, null, null, ssrUniqueId, ssrBlockState, null, null, retryCount ?? null];
...
const angularCoreEnv = (() => ({
  ...
  'ɵɵdeferEnableRetry': ɵɵdeferEnableRetry,
